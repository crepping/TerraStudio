...
$in="update fusion set f_titulodominio = '$f_titulodominio', f_incripcion ='$f_incripcion',f_certificadonumero = '$f_certificadonumero', f_certificadodominio = '$f_dominio', 
f_cedula = '$f_cedula', f_podernotarial = '$notario', f_siidetallado = '$interno', comentario = '$dir' Where id_fusion='$cod'";   
...
if (!$dato) {
  ...
  setTimeout(function() {
      swal({
          position: 'top-center',
          icon: 'error',
          title: 'Error al Modificar la Fusion',
          buttons: false,
          timer: 1500
      }).then(() => {
          window.location = 'Vista_Fusion.php';
      });
  }, 1500);
</script>";
}else{
...
echo "<script>
        setTimeout(function() {
            swal({
                position: 'top-center',
                icon: 'success',
                title: 'Proyecto Fusion Modificado Correctamente',
                buttons: false,
                timer: 1500
            }).then(() => {
                window.location = 'Vista_Fusion.php';
